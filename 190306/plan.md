...
* Пример: дерево отрезков для подсчёта суммы на отрезке
  * Абстрагировались от конкретного решения, чтобы выделить интерфейс задачи "сумма на отрезке"
  * `__init__`
  * `get(l, r)` или даже `__getitem__` со `slice`
  * Дальше можно добавлять групповые операции::
    * Опционально `__setitem__` со `slice`
  * Можно требовать либо `+` для элементов, либо потребовать
    отдельную функцию для складывания и нейтральный элемент
